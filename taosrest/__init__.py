...
def connect(**kwargs) -> TaosRestConnection:
   ...
   - timezone: str | datetime.tzinfo, optional, default None.
        When convert_timestamp is true, which timezone to use in the datetime object.
        When the type is str, it should be recognized by [pytz package](https://pypi.org/project/pytz/).
        If the value is None, system timezone will be used and the returned datetime object will be offset-naive (no tzinfo), otherwise the returned datetime will be offset-aware(with tzinfo)
   Examples
   -----------------------------
