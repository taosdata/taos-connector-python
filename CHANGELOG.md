...
-  close debug log for default

## v2.7.20 - 2024-12-30

### Features:

-  add 3.0 branch to check ci
-  add check insert data correct
-  add checkdata type and range
-  add geometry datatype test
-  change version to 2.8.0
-  init with row mode test case
-  split statement2 from statement file

### Bug Fixes:

-  TAOS_FIELD_COL write TAOS_FIELD_TAG mistake
-  TDengine branch swith to 3.0 from main
-  add check get fields data correct case
-  add check timestamp range and add except check frame
-  add debug1 debug2 debug3 diff level info
-  add evn load dll path
-  add evn load dll path1
-  add evn load dll path2
-  add evn load dll path3
-  add evn load dll path4
-  add filed_type to fields clas
-  add get field
-  add none value test
-  add normal table insert case
-  add rust and go connector
-  add the main function for easy debugging
-  all string type encode with utf-8
-  bind with tables buf is fixed
-  bind2 support timezone setting
-  bind2.py add set_value function
-  bind_param method adds type check for the iterated type of tbname
-  change branch to 3.0
-  check bindv arrar count consistent
-  checkCorrect add float compare
-  ci case failed
-  comment insert_bind_with_table call
-  comment use get_field because engine crash
-  compare class with member same
-  ensure name unique
-  exclude rest and print taosadapter show running statu
-  exclude rest for test
-  github workflow action@3 upgrade to action@4
-  hyper link for doc reference 404
-  linux-cross artifact name unique
-  mac branch switch to 3.0 branch
-  merge main change
-  merge main to 3.0 1217
-  migrate gettext install to first
-  migrate the get-meta-api-call  to the prepare method
-  modify all to 3.0 branch
-  name dup
-  name use matrix.target name
-  no need to get meta info when tbname not bind
-  pass test_query_a.py since core
-  pip3 install szlocal
-  prepare add normal table create arg
-  remove DVERNUMBER
-  remove bind2 flag is_ignore_update
-  remove ls /usr/local/lib
-  remove rest exclude
-  reset default time zone
-  restore murmurhash3_32 modify by mistake
-  set_value remove length param
-  set_value set precision to third param
-  stmt v1 bind suport timezone setting
-  stmt2 init reqid type check
-  stmt2 init reqid uses random number
-  stmt2 init reqid uses thread-level self-adding variables when not provided
-  stmt2 query basic test passed
-  sudo brew install
-  support float covert bool
-  supported type of varbinary and geometry add bytearray in func checkTypeValid
-  sync mac.yml from 3.0
-  uniform build machine
-  upgrade version 4
-  upload name same
-  use macos-13 on mac platform
-  varbinary with bytearray
-  version conflict for version.py
-  when get-meta-api-call failed, error message optimization
-  when sql is not str type，raise exception
-  when sql is null or empty, raise exception
-  wkt write to wkb

### Enhancements:

-  add get fields logic
-  add precision covert timestamp type from get field
-  error deal
-  get fields with new replace
-  interface is impl finish
-  move field_type define from field_v3.py to cinterface.py
-  statement2 is define ok
-  support interface with table finish
-  taos_stmt2_get_fields add count returned value
-  taos_stmt2_get_fields optimize test case
- remove log info

### Tests:

-  add bind2 test case
-  add bind_base test case
-  add make install
-  comment out test cases that are not supported by the engine
-  default time zone reads the local environment
-  refactor the stmt2 test test case so that its prerequisites do not depend on the stmt2 insert
-  show taosadapter exist
-  split taosadapter alone run
