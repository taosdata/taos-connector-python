...
@pytest.mark.skip(reason="know bug TD-16994")
def test_bind_function_param():
    conn = connect()
    queryparam = new_bind_params(1)
    queryparam[0].int(3)
    queryStat = conn.statement("select bottom(bu,?) from log group by bu;")
    queryStat.bind_param(queryStat)
    queryStat.execute()
